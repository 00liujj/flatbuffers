...
template<typename T> class Vector {
 ...
 private:
  ...
  template<typename K> static int KeyCompare(const void *ap, const void *bp) {
    ...
    T table = IndirectHelper<T>::Read(data, 0);
...
class vector_downward {
 ...
 private:
  ...
  void reallocate(size_t len) {
    ...
    uoffset_t old_scratch_size = scratch_size();
...
class FlatBufferBuilder {
 public:
  ...
  uoffset_t EndTable(uoffset_t start) {
    ...
    for (uint8_t* it = buf_.scratch_end() - num_field_loc * sizeof(FieldLoc);
         it < buf_.scratch_end(); it += sizeof(FieldLoc)) {
      FieldLoc * field_location = reinterpret_cast<FieldLoc *>(it);
    ...
    if (dedup_vtables_) {
      for (uint8_t * it = buf_.scratch_data(); it < buf_.scratch_end();
           it += sizeof(uoffset_t)) {
        uoffset_t * vt_offset_ptr = reinterpret_cast<uoffset_t *>(it);
        voffset_t * vt2 = reinterpret_cast<voffset_t *>(buf_.data_at(*vt_offset_ptr));
  ...
  Offset<String> CreateSharedString(const char *str, size_t len) {
    ...
    uoffset_t size_before_string = buf_.size();
    // Must first serialize the string, since the set is all offsets into
    // buffer.
    Offset<String> off = CreateString(str, len);
    StringOffsetMap::const_iterator it = string_pool->find(off);
  ...
  Offset<Vector<Offset<T> > > CreateVector(const Offset<T> *v, size_t len) {
    ...
    for (size_t i = len; i > 0;) { PushElement(v[--i]); }
  ...
  uoffset_t CreateUninitializedVector(size_t len, size_t elemsize,
    ...
    uoffset_t vec_start = GetSize();
    uoffset_t vec_end = EndVector(len);
 ...
 protected:
  ...
  struct StringOffsetCompare {
    ...
    bool operator()(const Offset<String> &a, const Offset<String> &b) const {
      const String * stra = reinterpret_cast<const String *>(buf_->data_at(a.o));
      const String * strb = reinterpret_cast<const String *>(buf_->data_at(b.o));
...
class Verifier FLATBUFFERS_FINAL_CLASS {
 public:
  ...
  bool VerifyVector(const uint8_t *vec, size_t elem_size,
    ...
    size_t max_elems = FLATBUFFERS_MAX_BUFFER_SIZE / elem_size;
  ...
  bool VerifyBufferFromStart(const char *identifier, const uint8_t *start) {
    ...
    uoffset_t o = VerifyOffset(start);
  ...
  uoffset_t VerifyOffset(const uint8_t *start) const {
    ...
    uoffset_t o = ReadScalar<uoffset_t>(start);
    Check(o != 0);
    return o;
  }
...
class Table {
 public:
  ...
  voffset_t GetOptionalFieldOffset(voffset_t field) const {
    ...
    const uint8_t * vtable = GetVTable();
...
inline const uint8_t *GetBufferStartFromRootPointer(const void *root) {
  const Table * table = reinterpret_cast<const Table *>(root);
  const uint8_t * vtable = table->GetVTable();
  // Either the vtable is before the root or after the root.
  const uint8_t * start = (std::min)(vtable, reinterpret_cast<const uint8_t *>(root));
  // Align to at least sizeof(uoffset_t).
  start = reinterpret_cast<const uint8_t *>(reinterpret_cast<size_t>(start) &
                                            ~(sizeof(uoffset_t) - 1));
  ...
  for (size_t possible_roots = FLATBUFFERS_MAX_ALIGNMENT / sizeof(uoffset_t) + 1;
